...
namespace Neanias.Accounting.Service.Web.Model
{
	...
	public class AccountBuilder
	{
		...
		public async Task<Account> Build(IFieldSet fields, ClaimsPrincipal principal)
		{
			...
			Neanias.Accounting.Service.Model.User user = null;
			...
			if (subjectId.HasValue)
			{
				IFieldSet userFields = new FieldSet(
					nameof(Neanias.Accounting.Service.Model.User.Id),
					new String[] { nameof(Neanias.Accounting.Service.Model.User.Profile), nameof(Neanias.Accounting.Service.Model.UserProfile.Timezone) }.AsIndexer(),
					new String[] { nameof(Neanias.Accounting.Service.Model.User.Profile), nameof(Neanias.Accounting.Service.Model.UserProfile.Language) }.AsIndexer(),
					new String[] { nameof(Neanias.Accounting.Service.Model.User.Profile), nameof(Neanias.Accounting.Service.Model.UserProfile.Culture) }.AsIndexer());
				user = await this._queryingService.FirstAsAsync(
					this._queryFactory.Query<UserQuery>()
						.DisableTracking()
						.Subject(subjectId.Value.ToString()),
					this._builderFactory.Builder<Neanias.Accounting.Service.Model.UserBuilder>(),
					userFields);
			}
