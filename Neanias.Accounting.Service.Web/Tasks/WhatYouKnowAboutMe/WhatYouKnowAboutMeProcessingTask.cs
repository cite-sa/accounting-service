...
namespace Neanias.Accounting.Service.Web.Tasks.WhatYouKnowAboutMe
{
					...
					using (var transaction = await dbContext.Database.BeginTransactionAsync())
					{
						try
						{
							...
							Data.WhatYouKnowAboutMe request = await queryFactory.Query<WhatYouKnowAboutMeQuery>()
								...
								.Ordering(new Ordering().AddDescending(nameof(Neanias.Accounting.Service.Model.WhatYouKnowAboutMe.CreatedAt)))
								.FirstAsync();
