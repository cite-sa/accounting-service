...
namespace Cite.Accounting.Service.Query
{
	...
	public class QueueInboxQuery : AsyncQuery<QueueInbox>
	{
		...
		protected override Task<IQueryable<QueueInbox>> ApplyFiltersAsync(IQueryable<QueueInbox> query)
		{
			...
			if (this._exchanges != null) query = query.Where(x => x.Exchange != null && this._exchanges.Contains(x.Exchange));
			if (this._queues != null) query = query.Where(x => x.Queue != null && this._queues.Contains(x.Queue));
			if (this._routes != null) query = query.Where(x => x.Route != null && this._routes.Contains(x.Route));
			if (this._applicationIds != null) query = query.Where(x => x.ApplicationId != null && this._applicationIds.Contains(x.ApplicationId));
