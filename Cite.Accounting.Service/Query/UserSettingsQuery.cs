...
namespace Cite.Accounting.Service.Query
{
	...
	public class UserSettingsQuery : AsyncQuery<UserSettings>
	{
		...
		protected override Task<IQueryable<UserSettings>> ApplyFiltersAsync(IQueryable<UserSettings> query)
		{
			...
			if (this._keys != null) query = query.Where(x => x.Key != null && this._keys.Contains(x.Key));
			if (this._names != null) query = query.Where(x => x.Name != null && this._names.Contains(x.Name));
