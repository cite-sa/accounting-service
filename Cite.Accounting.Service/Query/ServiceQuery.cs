...
namespace Cite.Accounting.Service.Query
{
	...
	public class ServiceQuery : AsyncQuery<Data.Service>
	{
		...
		protected override async Task<IQueryable<Data.Service>> ApplyAuthzAsync(IQueryable<Data.Service> query)
		{
		...
		protected override Task<IQueryable<Data.Service>> ApplyFiltersAsync(IQueryable<Data.Service> query)
		{
			...
			if (!String.IsNullOrEmpty(this._like)) query = query.Like(this._config.Provider, this._like, x => x.Code, x => x.Name, x => x.Description);
