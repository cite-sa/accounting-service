...
namespace Cite.Accounting.Service.Query
{
	...
	public class MetricQuery : AsyncQuery<Metric>
	{
		...
		protected override async Task<IQueryable<Metric>> ApplyAuthzAsync(IQueryable<Metric> query)
		{
			if (this._authorize.Contains(AuthorizationFlags.None)) return query;
			if (this._authorize.Contains(AuthorizationFlags.Permission) && await this._authorizationContentResolver.HasPermission(Permission.BrowseMetric)) return query;

			IEnumerable<Guid> serviceIds = new List<Guid>();
			if (this._authorize.Contains(AuthorizationFlags.Sevice)) serviceIds = await this._authorizationContentResolver.AffiliatedServicesAsync(Permission.BrowseMetric) ?? new List<Guid>();
		...
		protected override Task<IQueryable<Metric>> ApplyFiltersAsync(IQueryable<Metric> query)
		{
			...
			if (!String.IsNullOrEmpty(this._like)) query = query.Like(this._config.Provider, this._like, x => x.Code);
