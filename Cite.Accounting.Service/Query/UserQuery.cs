...
namespace Cite.Accounting.Service.Query
{
	...
	public class UserQuery : AsyncQuery<User>
	{
		...
		protected override Task<IQueryable<User>> ApplyFiltersAsync(IQueryable<User> query)
		{
			...
			if (!String.IsNullOrEmpty(this._like)) query = query.Like(this._config.Provider, this._like, x => x.Name, x => x.Email, x => x.Subject);
			...
			if (this._issuersExact != null) query = query.Where(x => x.Issuer != null && this._issuersExact.Contains(x.Issuer));
			if (this._subjectsExact != null) query = query.Where(x => x.Subject != null && this._subjectsExact.Contains(x.Subject));
