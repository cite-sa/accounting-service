...
namespace Cite.Accounting.Service.Service.UserSettings
{
	public class UserSettingsService : IUserSettingsService
	{
		...
		public async Task<Model.UserSettings> DeleteAndSaveAsync(Guid userId, Guid key)
		{
			this._logger.Debug("deleting setttings {userid} - {key}", userId, key);

			await this._authService.AuthorizeOwnerForce(new OwnedResource(userId));

			List<Data.UserSettings> deletedDatas = await this._deleterFactory.Deleter<Model.UserSettingsDeleter>().Delete(userId, key);
