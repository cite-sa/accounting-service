...
namespace Cite.Accounting.Service.Service.HierarchyResolver
{
	...
	public class HierarchyResolverService : IHierarchyResolverService
	{
		...
		public async Task<IEnumerable<Guid>> ResolveChildServices(Guid parentId)
		{
			...
			HashSet<Guid> childs = await this.GetChilds(itemType, parentId, async (id) => await this._queryFactory.Query<ServiceQuery>().ParentIds(id).IsActive(IsActive.Active).DisableTracking().CollectAsync(x => x.Id));
		...
		public async Task<ChildParents> ResolveParentServices(Guid childId)
		{
			...
			ChildParents parents = await this.GetParents<Data.Service>(itemType, item, (item) => item.Id, (item) => item.ParentId,
				async (id) => await this._queryFactory.Query<ServiceQuery>().Ids(id).IsActive(IsActive.Active).DisableTracking().FirstAsync(x => new Data.Service() { Id = x.Id, ParentId = x.ParentId }));
		...
		public async Task<IEnumerable<Guid>> ResolveChildServiceResources(Guid parentId)
		{
			...
			HashSet<Guid> childs = await this.GetChilds(itemType, parentId, async (id) => await this._queryFactory.Query<ServiceResourceQuery>().IsActive(IsActive.Active).ParentIds(id).DisableTracking().CollectAsync(x => x.Id));
		...
		public async Task<ChildParents> ResolveParentServiceResources(Guid childId)
		{
			...
			ChildParents parents = await this.GetParents<Data.ServiceResource>(itemType, item, (item) => item.Id, (item) => item.ParentId,
				async (id) => await this._queryFactory.Query<ServiceResourceQuery>().Ids(id).IsActive(IsActive.Active).DisableTracking().FirstAsync(x => new Data.ServiceResource() { Id = x.Id, ParentId = x.ParentId }));
		...
		public async Task<IEnumerable<Guid>> ResolveChildServiceActions(Guid parentId)
		{
			...
			HashSet<Guid> childs = await this.GetChilds(itemType, parentId, async (id) => await this._queryFactory.Query<ServiceActionQuery>().IsActive(IsActive.Active).ParentIds(id).DisableTracking().CollectAsync(x => x.Id));
		...
		public async Task<ChildParents> ResolveParentServiceActions(Guid childId)
		{
			...
			ChildParents parents = await this.GetParents<Data.ServiceAction>(itemType, item, (item) => item.Id, (item) => item.ParentId,
				async (id) => await this._queryFactory.Query<ServiceActionQuery>().Ids(id).IsActive(IsActive.Active).DisableTracking().FirstAsync(x => new Data.ServiceAction() { Id = x.Id, ParentId = x.ParentId }));
