...
namespace Cite.Accounting.Service.Authorization
{
	public class AuthorizationContentResolver : IAuthorizationContentResolver
	{
		...
		public async Task<Boolean> HasPermission(params String[] permissions) => await this._authorizationService.Authorize(permissions);
		...
		private async Task<IEnumerable<Guid>> ResolveAffiliatedServices(params String[] permissions)
		{
			...
			{
				UserRolePermissionMapping mapping = userRolePermissionMappings.FirstOrDefault(x => x.RoleId == serviceRole.RoleId);
		...
		public async Task<AffiliatedResource> ServiceResourceAffiliation(Guid serviceResourceId)
		{
			...
			AffiliatedResource resource = await this.ServiceAffiliation(serviceId);
		...
		public async Task<AffiliatedResource> ServiceUserAffiliation(Guid id)
		{
			...
			ElasticResponseItem<Elastic.Data.UserInfo> userInfo = await this._queryFactory.Query<UserInfoQuery>().Ids(id).FirstAsync();

			Guid serviceId = await this._dbContext.Services.AsNoTracking().Where(x => x.Code == userInfo.Item.ServiceCode).Select(x => x.Id).FirstOrDefaultAsync();
