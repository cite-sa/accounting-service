...
namespace Cite.Accounting.Service.Model
{
	public abstract class Builder<M, D> : IBuilder where D : class
	{
		...
		public async Task<M> Build(IFieldSet fields, D data)
		{
			if (data == null)
			{
				this._logger.Debug(new MapLogEntry("requested build for null item requesting fields").And("fields", fields));
				return default(M);
			}
			List<M> models = await this.Build(fields, new D[] { data });
			return models.FirstOrDefault();
		}

		public abstract Task<List<M>> Build(IFieldSet fields, IEnumerable<D> datas);

		public async Task<Dictionary<K, M>> AsForeignKey<K>(Query<D> query, IFieldSet fields, Func<M, K> keySelector)
		{
			this._logger.Trace("Building references from query");
			List<D> datas = await query.CollectAsync(fields);
			this._logger.Debug("collected {count} items to build", datas?.Count);
			return await this.AsForeignKey(datas, fields, keySelector);
		}

		public async Task<Dictionary<K, M>> AsForeignKey<K>(IEnumerable<D> datas, IFieldSet fields, Func<M, K> keySelector)
		{
			this._logger.Trace("building references");
			List<M> models = await this.Build(fields, datas);
		...
		public async Task<Dictionary<K, List<M>>> AsMasterKey<K>(Query<D> query, IFieldSet fields, Func<M, K> keySelector)
		{
			this._logger.Trace("Building details from query");
			List<D> datas = await query.CollectAsync(fields);
			this._logger.Debug("collected {count} items to build", datas?.Count);
			return await this.AsMasterKey(datas, fields, keySelector);
		}

		public async Task<Dictionary<K, List<M>>> AsMasterKey<K>(IEnumerable<D> datas, IFieldSet fields, Func<M, K> keySelector)
		{
