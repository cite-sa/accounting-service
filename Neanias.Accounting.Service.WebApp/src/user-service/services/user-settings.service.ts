...
export class UserSettingsService extends BaseService {
	...
	remove<T>(id: Guid, userSettingsInformation: UserSettingsInformation<T> = null) {
		this.deleteFromWebServer(id, userSettingsInformation, true);
	}

	private deleteFromWebServer<T>(id: Guid, userSettingsInformation: UserSettingsInformation<T>, updateLocalAfterPush: boolean = true) {
		this.userSettingsHttpService.delete(id).pipe(takeUntil(this._destroyed)).subscribe(items => {
