...
{
	public class AccountingAggregateResultItemCensor : Censor
	{
		private readonly CensorFactory _censorFactory;
		private readonly IAuthorizationService _authService;
		private readonly ILogger<AccountingAggregateResultItemCensor> _logger;

		public AccountingAggregateResultItemCensor(
			CensorFactory censorFactory,
			IAuthorizationService authService,
			ILogger<AccountingAggregateResultItemCensor> logger)
		{
			this._logger = logger;
		...
		public async Task Censor(IFieldSet fields, Guid? userId = null)
		{
			...
			await this._authService.AuthorizeOrOwnerForce(userId.HasValue ? new OwnedResource(userId.Value) : null, Permission.CalculateAccountingInfo, Permission.DeferredAffiliation);
