...
{
	public class AccountingAggregateResultGroupBuilder : Builder<AccountingAggregateResultGroup, Elastic.Query.AggregateResultGroup>
	{
		...
		public override async Task<List<AccountingAggregateResultGroup>> Build(IFieldSet fields, IEnumerable<Elastic.Query.AggregateResultGroup> datas)
		{
			...
			foreach (Elastic.Query.AggregateResultGroup d in datas)
			{
		...
		private ItemCodeMap GetItemCodeMap(Elastic.Query.AggregateResultGroup d, string codeKey) => new ItemCodeMap() { Code = this.ExtractValue(d, codeKey), ServiceCode = this.ExtractValue(d, nameof(Model.AccountingEntry.Service)) };

		private async Task<Dictionary<String, Service>> CollectServices(IFieldSet fields, IEnumerable<Elastic.Query.AggregateResultGroup> datas)
		{
		...
		private async Task<Dictionary<ItemCodeMap, ServiceResource>> CollectServiceResources(IFieldSet fields, IEnumerable<Elastic.Query.AggregateResultGroup> datas)
		{
			if (fields.IsEmpty() || !datas.Any()) return null;
		...
		private async Task<Dictionary<ItemCodeMap, ServiceAction>> CollectServiceActions(IFieldSet fields, IEnumerable<Elastic.Query.AggregateResultGroup> datas)
		{
			if (fields.IsEmpty() || !datas.Any()) return null;
		...
		private async Task<Dictionary<ItemCodeMap, UserInfo>> CollectUserInfos(IFieldSet fields, IEnumerable<Elastic.Query.AggregateResultGroup> datas)
		{
			if (fields.IsEmpty() || !datas.Any()) return null;
		...
		private String ExtractValue(Elastic.Query.AggregateResultGroup group, String fieldName)
		{
