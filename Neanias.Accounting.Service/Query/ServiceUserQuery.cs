...
namespace Neanias.Accounting.Service.Query
{
	...
	public class ServiceUserQuery : Query<ServiceUser>
	{
		...
		protected override IQueryable<ServiceUser> ApplyAuthz(IQueryable<ServiceUser> query)
		{
			...
			Expression<Func<ServiceUser, Boolean>> filterPredicate = x =>
				(
					(ownerId.HasValue ? x.UserId == ownerId : false)
				)
				||
				(serviceIds.Any() ? serviceIds.Contains(x.ServiceId) : false);

			query = query.Where(filterPredicate);

			return query;
		}
		protected override IQueryable<ServiceUser> ApplyFilters(IQueryable<ServiceUser> query)
		{
