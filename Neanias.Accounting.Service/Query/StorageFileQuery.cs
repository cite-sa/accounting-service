...
namespace Neanias.Accounting.Service.Query
{
	...
	public class StorageFileQuery : Query<StorageFile>
	{
		...
		protected override IQueryable<StorageFile> ApplyFilters(IQueryable<StorageFile> query)
		{
			...
			if (this._whatYouKnowAboutMeQuery != null)
			{
				...
				IQueryable<Guid?> subQuery = this.BindSubQuery(this._whatYouKnowAboutMeQuery, this._dbContext.WhatYouKnowAboutMes, y => y.StorageFileId).Distinct();
