...
export class ServiceResourceEditorModel extends BaseEditorModel implements ServiceResourcePersist {
	name: string;
	code: string;
	description: string;
	parentId: Guid;
	...
	constructor(private authService: AuthService) {
		...
		this.canEdit = this.authService.hasPermission(AppPermission.NewServiceResource);
		this.canDelete = this.authService.hasPermission(AppPermission.DeleteServiceResource);
	}

	public fromModel(item: ServiceResource): ServiceResourceEditorModel {
		if (item) {
			...
			this.canEdit = this.authService.hasPermission(AppPermission.EditServiceResource) || item.authorizationFlags?.some(x => x === AppPermission.EditServiceResource);
			this.canDelete = this.authService.hasPermission(AppPermission.DeleteServiceResource) || item.authorizationFlags?.some(x => x === AppPermission.DeleteServiceResource);
