...
import { ServiceActionFilter } from '@app/core/query/service-action.lookup';
import { ServiceActionService } from '@app/core/services/http/service-action.service';
import { Lookup } from '@common/model/lookup';

@Component({
	selector: 'app-service-action-listing-filters',
	templateUrl: './service-action-listing-filters.component.html',
	styleUrls: ['./service-action-listing-filters.component.scss']
})
export class ServiceActionListingFiltersComponent extends BaseComponent implements OnInit, OnChanges {

	@Input() key = { key: 'ServiceActionListingUserSettings' };
	@Input() lookup: Lookup;
	@Input() filter: ServiceActionFilter;
	@Input() autoSelectUserSettings: boolean = false;

	@Output() filterChange = new EventEmitter<ServiceActionFilter>();
	...
	//
	visibleFields(filter: ServiceActionFilter) {
