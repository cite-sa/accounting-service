...
namespace Cite.Accounting.Service.Web.Controllers
{
	...
	public class ServiceSyncController : ControllerBase
	{
		...
		[HttpPost("query")]
		[Authorize]
		public async Task<QueryResult<Cite.Accounting.Service.Model.ServiceSync>> Query([FromBody] ServiceSyncLookup lookup)
		{
			...
			List<Cite.Accounting.Service.Model.ServiceSync> models = await this._queryingService.CollectAsAsync(query, this._builderFactory.Builder<ServiceSyncBuilder>().Authorize(Accounting.Service.Authorization.AuthorizationFlags.OwnerOrPermissionOrSevice), lookup.Project);
			...
			return new QueryResult<Cite.Accounting.Service.Model.ServiceSync>(models, count);
		}

		[HttpGet("{id}")]
		[Authorize]
		public async Task<Cite.Accounting.Service.Model.ServiceSync> Get([FromRoute] Guid id, [ModelBinder(Name = "f")] IFieldSet fieldSet)
		{
			...
			Cite.Accounting.Service.Model.ServiceSync model = await this._queryingService.FirstAsAsync(query, this._builderFactory.Builder<ServiceSyncBuilder>().Authorize(Accounting.Service.Authorization.AuthorizationFlags.OwnerOrPermissionOrSevice), fieldSet);
			if (model == null) throw new MyNotFoundException(this._localizer["General_ItemNotFound", id, nameof(Cite.Accounting.Service.Model.ServiceSync)]);
		...
		public async Task<Cite.Accounting.Service.Model.ServiceSync> Persist([FromBody] ServiceSyncPersist model, [ModelBinder(Name = "f")] IFieldSet fieldSet)
		{
			this._logger.Debug(new MapLogEntry("persisting").And("model", model).And("fields", fieldSet));

			Cite.Accounting.Service.Model.ServiceSync persisted = await this._serviceSyncervice.PersistAsync(model, fieldSet);
