...
namespace Cite.Accounting.Service.Web.Controllers
{
	...
	public class UserController : ControllerBase
	{
		...
		[HttpPost("query")]
		[Authorize]
		public async Task<QueryResult<Cite.Accounting.Service.Model.User>> Query([FromBody] UserLookup lookup)
		{
			...
			List<Cite.Accounting.Service.Model.User> models = await this._queryingService.CollectAsAsync(query, this._builderFactory.Builder<UserBuilder>().Authorize(Accounting.Service.Authorization.AuthorizationFlags.OwnerOrPermissionOrSevice), lookup.Project);
			...
			return new QueryResult<Cite.Accounting.Service.Model.User>(models, count);
		}

		[HttpGet("{id}")]
		[Authorize]
		public async Task<Cite.Accounting.Service.Model.User> Get([FromRoute] Guid id, [ModelBinder(Name = "f")] IFieldSet fieldSet)
		{
			...
			Cite.Accounting.Service.Model.User model = await this._queryingService.FirstAsAsync(query, this._builderFactory.Builder<UserBuilder>().Authorize(Accounting.Service.Authorization.AuthorizationFlags.OwnerOrPermissionOrSevice), fieldSet);
			if (model == null) throw new MyNotFoundException(this._localizer["General_ItemNotFound", id, nameof(Cite.Accounting.Service.Model.User)]);
		...
		public async Task<Cite.Accounting.Service.Model.User> Persist([FromBody] UserPersist model, [ModelBinder(Name = "f")] IFieldSet fieldSet)
		{
			this._logger.Debug(new MapLogEntry("persisting").And("model", model).And("fields", fieldSet));

			Cite.Accounting.Service.Model.User persisted = await this._userService.PersistAsync(model, fieldSet);
		...
		[ServiceFilter(typeof(TenantTransactionFilter))]
		public async Task<Cite.Accounting.Service.Model.User> Persist([FromBody] UserServiceUsersPersist model, [ModelBinder(Name = "f")] IFieldSet fieldSet)
		{
			this._logger.Debug(new MapLogEntry("persisting").And("model", model).And("fields", fieldSet));
		...
		public async Task<Cite.Accounting.Service.Model.User> UpdateName([FromBody] NamePatch model, [ModelBinder(Name = "f")] IFieldSet fieldSet)
		{
