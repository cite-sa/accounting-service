...
namespace Cite.Accounting.Service.Web.Controllers
{
	...
	public class ServiceController : ControllerBase
	{
		...
		[HttpPost("query")]
		[Authorize]
		public async Task<QueryResult<Cite.Accounting.Service.Model.Service>> Query([FromBody] ServiceLookup lookup)
		{
			...
			List<Cite.Accounting.Service.Model.Service> models = await this._queryingService.CollectAsAsync(query, this._builderFactory.Builder<ServiceBuilder>().Authorize(Accounting.Service.Authorization.AuthorizationFlags.OwnerOrPermissionOrSevice), lookup.Project);
			...
			return new QueryResult<Cite.Accounting.Service.Model.Service>(models, count);
		}

		[HttpGet("{id}")]
		[Authorize]
		public async Task<Cite.Accounting.Service.Model.Service> Get([FromRoute] Guid id, [ModelBinder(Name = "f")] IFieldSet fieldSet)
		{
			...
			Cite.Accounting.Service.Model.Service model = await this._queryingService.FirstAsAsync(query, this._builderFactory.Builder<ServiceBuilder>().Authorize(Accounting.Service.Authorization.AuthorizationFlags.OwnerOrPermissionOrSevice), fieldSet);
			if (model == null) throw new MyNotFoundException(this._localizer["General_ItemNotFound", id, nameof(Cite.Accounting.Service.Model.Service)]);
		...
		public async Task<Cite.Accounting.Service.Model.Service> Persist([FromBody] ServicePersist model, [ModelBinder(Name = "f")] IFieldSet fieldSet)
		{
			this._logger.Debug(new MapLogEntry("persisting").And("model", model).And("fields", fieldSet));

			Cite.Accounting.Service.Model.Service persisted = await this._serviceService.PersistAsync(model, fieldSet);
